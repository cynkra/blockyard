...
The v1 integration system lets users manually enroll credentials for
external services (e.g., an OpenAI API key). Some services require
**system-provisioned** credentials instead — the server creates an account
or access key on the user's behalf, rather than the user supplying one.

The primary motivating use case is per-user storage for blockr board
save/restore. Each user needs their own credential pair (e.g., S3 access
key + secret) scoped to their own data. These credentials cannot be
user-provided — they must be generated by calling the storage backend's
admin API.

#### Design: Exec Hooks

Blockyard executes a configured command once per new user. The command
provisions whatever external resources the user needs and returns
credentials as JSON on stdout. Blockyard writes the result to OpenBao.

This keeps blockyard decoupled from any specific storage backend — it
doesn't need to know how to talk to MinIO, AWS IAM, or a database. The
hook script is the adapter.
...
id      = "s3"
label   = "S3 Storage"
command = "/etc/blockyard/hooks/provision-s3.sh"
```

Multiple hooks can be configured. Each runs independently.

##### Execution Contract

**Trigger:** once per user, on first OIDC login. Blockyard checks whether
credentials already exist in OpenBao at `secret/data/users/{sub}/apikeys/{id}`
before firing — if present, the hook is skipped.

**Input:** user context via environment variables:

| Variable                  | Value                        |
|---------------------------|------------------------------|
| `BLOCKYARD_HOOK_SUB`     | OIDC `sub` claim             |
| `BLOCKYARD_HOOK_EMAIL`   | User's email                 |
| `BLOCKYARD_HOOK_NAME`    | User's display name          |
| `BLOCKYARD_HOOK_ID`      | Hook ID from config (e.g., `s3`) |

**Output:** JSON object on stdout. All keys and values are strings.
Example for S3:

```json
{
  "access_key": "user-001-ak",
  "secret_key": "user-001-sk",
  "bucket": "blockyard-user-001",
  "endpoint": "http://minio:9000"
}
```

The response format is opaque to blockyard — it stores the entire JSON
object as-is in OpenBao. The R app (blockr) is responsible for knowing
which keys to read.

**Storage path:** `secret/data/users/{sub}/apikeys/{id}` — the same path
pattern used by the existing manual credential enrollment, so the read
side (vault token injection, R helper function) works without changes.

**Error handling:** non-zero exit code means failure. Stderr is logged
at `warn` level. The user is not marked as onboarded for this hook, and
blockyard retries on the next login.

##### Example: MinIO Provisioning Hook

A minimal hook script for the hello-blockr example:

```bash
#!/bin/sh
set -eu

# Create a MinIO user scoped to a personal bucket
mc admin user add local "$BLOCKYARD_HOOK_SUB" "$(openssl rand -hex 16)"
mc mb --ignore-existing "local/user-${BLOCKYARD_HOOK_SUB}"
mc admin policy attach local readwrite --user "$BLOCKYARD_HOOK_SUB"

# Return credentials as JSON
cat <<EOF
{
  "access_key": "$BLOCKYARD_HOOK_SUB",
  "secret_key": "...",
  "bucket": "user-${BLOCKYARD_HOOK_SUB}",
  "endpoint": "http://minio:9000"
}
EOF
```

In the example deployment, this script runs inside the blockyard container
(or a sidecar with `mc` pre-installed). In production, the same hook
pattern works with any backend — swap the script for one that calls AWS
IAM, creates a database user, provisions a Git repo, etc.

##### Future: Webhook Variant

A `webhook` field (mutually exclusive with `command`) could call an HTTP
endpoint instead of exec'ing a local command. This requires authentication
between blockyard and the webhook (mTLS or a shared secret), so it is
deferred. The exec model covers all single-node deployments; webhooks
become relevant for multi-node setups where the provisioning service runs
on a different host.

##### Open Questions

1. **Credential rotation.** The current design provisions credentials once
   and never updates them. For long-lived deployments, credentials may need
   rotation. Options: a periodic re-run of the hook (with the hook script
   handling "update existing" vs. "create new"), a separate `rotate`
   trigger, or delegation to the storage backend's own rotation mechanism
   (e.g., MinIO's built-in key rotation). Deferred until closer to
   implementation.

2. **Teardown.** Should there be a corresponding hook for user deletion
   or deactivation? This would clean up external resources (delete the
   MinIO user, drop the database account). Not needed for v2 — manual
   cleanup is acceptable — but the hook model extends naturally to a
   `user.delete` trigger.
