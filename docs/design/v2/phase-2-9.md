# Phase 2-9: Multi-Page Navigation + htmx
...
Depends on phase 2-8 (backend prerequisites + CLI) for the API
endpoints consumed by the Deployment History page and Profile page.
...
icon on each card opens the per-app management sidebar (phase 2-10).
...
phase 2-10 registers the fragment routes:
...
| Column | Content |
|--------|---------|
| App | App name (links to app card sidebar in phase 2-10) |
...
The CLI's `by login` command opens `{server}/profile#tokens` to direct
users here for token creation.
...
Phase 2-10 adds many more fragment templates and extends the `UI` struct
...
**Sidebar shell** (non-functional until phase 2-10):
...
Minimal vanilla JS for sidebar open/close (ready for phase 2-10):
...
buttons. Add `id="tokens"` anchor on the PAT section.
...
JS and CSS transitions. Non-functional until phase 2-10 registers
fragment routes.
