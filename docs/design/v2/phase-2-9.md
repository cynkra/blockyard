...
in phase 2-8. The sidebar provides six tabs (Overview, Settings,
...
Depends on phase 2-8 (backend prerequisites + navigation).
...
The `hx-patch` attributes target `PATCH /api/v1/apps/{id}` (phase 2-8
...
The collaborators list uses `ListAppAccessWithNames()` (added in
phase 2-8) to join `app_access` with the `users` table for
human-readable display names. If a granted principal has never logged
in, the raw principal (OIDC subject) is shown instead.
...
1. **Fragment route scaffolding** — register `/ui/` routes, extract
