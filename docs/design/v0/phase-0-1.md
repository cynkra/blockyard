...
impl Config {
    ...
    fn validate(&self) -> Result<(), ConfigError> {
        ...
        Self::ensure_dir_writable(&self.storage.bundle_server_path, "storage.bundle_server_path")?;
    ...
    fn ensure_dir_writable(path: &std::path::Path, label: &str) -> Result<(), ConfigError> {
        std::fs::create_dir_all(path).map_err(|e| {
            ConfigError::Validation(format!("{label}: cannot create directory '{}': {e}", path.display()))
        })?;
        let test_file = path.join(".blockyard-write-test");
        std::fs::write(&test_file, b"").map_err(|e| {
            ConfigError::Validation(format!("{label}: directory '{}' is not writable: {e}", path.display()))
        })?;
