...
func main() {
	...
	// others. Declared up here so the OpenBao renewer goroutine (started
	...
	// ── Initialize OpenBao (before DB so the Postgres auth-failure
	// fallback can self-mint dynamic creds, and before OIDC for vault
	...
	// ── Initialize database ──
	//
	// If OpenBao is configured with a database-secrets role
	// (database.vault_creds_path), install a rotator that self-mints
	// dynamic Postgres credentials on auth failure. The lease inherits
	// blockyard's own AppRole token lifetime, so it survives as long as
	// blockyard does — unlike deploy-side creds whose lease is revoked
	// when the provisioner token dies.
	var dbOpts []db.OpenOption
	if vaultClient != nil && cfg.Database.VaultCredsPath != "" {
		credsRole := cfg.Database.VaultCredsPath
		dbOpts = append(dbOpts, db.WithCredsRotator(db.RotatorFunc(
			func(ctx context.Context) (string, string, error) {
				u, p, _, err := vaultClient.GenerateDBCreds(ctx, vaultAdminToken(), credsRole)
				return u, p, err
			})))
	}
